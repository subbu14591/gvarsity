...
Modified for VenexusWebControls: Aug 27, 2008
By: Virgil Reboton
...
jQuery.fn.rating = function(url, options) {
    ...
    if (i == 0) {
        if(settings.cancel == true && settings.showcancel == 1){
    ...
    {
        var div = jQuery('<div class="star"></div>').append('<a href="#'+i+'" title="Give it '+i+'/'+settings.maxvalue+'">'+i+'</a>').appendTo(container);
...
jQuery.fn.get_Rating = function() {
        alert(settings.curvalue);
    return settings.curvalue;
};

jQuery.fn.set_ReadOnly = function(val) {
    settings.cancel = val;
};
...
stars.mouseover(function(){event.drain();event.fill(this);}).mouseout(function(){event.drain();event.reset();}).focus(function(){event.drain();event.fill(this);}).blur(function(){event.drain();event.reset();});
